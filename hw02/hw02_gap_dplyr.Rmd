...
con_sum <- count(con_only, continent) %>%  # dplyr: table of counts per continent
  as_tibble()
knitr::kable(summary(con_sum)) # Print summary of the data
...
knitr::kable(summary(esoph_cancer))
```
...
Because of the inclusion of a list, the function will go along the year-sorted data, taking turns selecting each country for each value of year. There are two ways to fix this code:
```{r recycling_fix}
method1 <- filter(gapminder, country %in% c("Rwanda","Afghanistan"))
method2 <- filter(gapminder, country == "Rwanda" | country == "Afghanistan")
DT::datatable(method1)
DT::datatable(method2)
```
